...
class PassTransferProxyTracker:
    """
    Weak pass-transfer proxy from the currently exposed player stream.

    This is NOT a true teammate-transfer detector.
    It only uses signals that are actually available in this wrapper:
      - current exposed player position
      - ball position / velocity
      - touch-like events

    Proxy logic:
      1. start an attempt on a touch + meaningful release
      2. require the ball to separate from the current player
      3. reward only if the ball is touched again after separation
         and traveled a minimum distance

    This tends to reward release-and-reacquire chains and is much weaker
    than a true two-player geometric pass detector, but it runs with the
    current wrapper interface.
    """

    MIN_RELEASE_SPEED = 0.80
    MIN_TRAVEL = 2.00
    MAX_STEPS = 18
    MIN_FREE_STEPS = 2
    SEPARATION_DIST = 1.80
    START_DIST = 1.50
    REATTACH_COOLDOWN = 2
    ...
    def update(
        ...
        *,
        ...
        dist_prev = float(np.linalg.norm(prev_player_pos - prev_ball_pos))
        ball_speed = float(np.linalg.norm(ball_vel))
        travel_from_prev = float(np.linalg.norm(ball_pos - prev_ball_pos))
        ...
        # Start a proxy attempt only on a real release-like event
        release_like = (
            touched_ball
            and dist_now <= self.START_DIST
        ...
        if self.pending is not None:
            ...
            pass_travel = float(
                np.linalg.norm(ball_pos - self.pending["release_ball_pos"])
            )

            # Require the ball to really leave the current player
            ...
            # Successful proxy completion:
            # the ball left, traveled enough, and there is another touch later.
            if (
                touched_ball
...
class CurriculumMetricsCallback(DefaultCallbacks):
    ...
    def on_episode_start(self, *, episode, **kwargs):
        ...
        episode.user_data["pass_reward_vals"] = []
        episode.user_data["pass_attempt_count_vals"] = []
        episode.user_data["pass_complete_count_vals"] = []
        episode.user_data["pass_fail_count_vals"] = []
        episode.user_data["pass_travel_vals"] = []
    ...
    def on_episode_step(self, *, episode, **kwargs):
        ...
        episode.user_data["pass_reward_vals"].append(_safe_float(info.get("pass_reward", 0.0)))
        episode.user_data["pass_attempt_count_vals"].append(_safe_float(info.get("pass_attempt_count", 0.0)))
        episode.user_data["pass_complete_count_vals"].append(_safe_float(info.get("pass_complete_count", 0.0)))
        episode.user_data["pass_fail_count_vals"].append(_safe_float(info.get("pass_fail_count", 0.0)))
        episode.user_data["pass_travel_vals"].append(_safe_float(info.get("pass_travel", 0.0)))
    ...
    def on_episode_end(self, *, episode, **kwargs):
        ...
        episode.custom_metrics["pass_reward_mean"] = float(
            np.mean(episode.user_data["pass_reward_vals"] or [0.0])
        )
        episode.custom_metrics["pass_travel_mean"] = float(
            np.mean(episode.user_data["pass_travel_vals"] or [0.0])
        )
        ...
        attempts = float(np.sum(episode.user_data["pass_attempt_count_vals"] or [0.0]))
        completes = float(np.sum(episode.user_data["pass_complete_count_vals"] or [0.0]))
        episode.custom_metrics["pass_completion_rate"] = (
            completes / attempts if attempts > 0 else 0.0
        )
    ...
    def on_train_result(self, *, result: dict, **kwargs):
        ...
        print(
            ...
            f"pass_reward_mean={cm.get('pass_reward_mean_mean')} "
            f"pass_attempts={cm.get('pass_attempt_count_mean')} "
            f"pass_completes={cm.get('pass_complete_count_mean')} "
            f"pass_completion_rate={cm.get('pass_completion_rate_mean')} "
        )
...
def _result_row(result: Dict[str, Any], stage: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "pass_completion_rate": cm.get("pass_completion_rate_mean"),
    }
...
def _save_training_outputs(analysis: tune.ExperimentAnalysis, output_dir: Path):
    ...
    tracked_columns = [
        ...
        "custom_metrics/pass_completion_rate_mean",
