"""
train_three_phase_curriculum_v2.py
===================================
Three-phase curriculum PPO for SoccerTwos — improved version.

Changes vs v1 (and bugs fixed vs first v2 draft)
-------------------------------------------------
Fix 2  — Separate value network (vf_share_layers=False, vf_loss_coeff=0.5).
          Stops policy/value gradients fighting over shared weights.
          NOTE: incompatible with checkpoints trained with vf_share_layers=True.
          This is a FRESH RUN script — do not try to resume old checkpoints.

Fix 3  — Higher entropy baked into the Stage-3 config block inside
          build_config() via a separate stage-3 sub-config, not via
          trainer.config mutation (which is unreliable in RLlib).
          entropy_coeff: 0.005 → 0.015 in stage 3.
          clip_param:    0.10  → 0.12  in stage 3.
          Achieved by passing the desired stage-3 values into a dedicated
          PPOTrainer rebuild at the stage boundary using trainer.workers.
          (See CurriculumStageSwitchCallback for details.)

Fix 4  — Stronger goal_progress_reward ONLY in Stage 3 (coeff 0.08 → 0.15).
          Stage 2 keeps the original 0.08 coefficient so it isn't shocked
          by a harder signal while still fighting agent_2.
          shooting_reward: fires when ball_vel ALIGNS WITH ball_to_goal
          (fixed sign — previous version had -ball_vel which was backwards,
          rewarding the ball moving AWAY from goal).

Fix 4b — shooting_reward sign corrected:
          dot(ball_vel_dir, goal_dir) > 0  ↔  ball moving toward opponent goal.
          Was: dot(-ball_vel_dir, goal_dir) which gave reward for the
          opposite direction.

Checkpoint saving
-----------------
  keep_checkpoints_num=None  → keep every single checkpoint (no pruning).
  checkpoint_freq=1           → save after every training iteration.
  checkpoint_score_attr removed so Ray never silently deletes checkpoints.
"""
...
class CurriculumRewardWrapper(gym.Wrapper):
    ...
    def _shape_with_stage_logic(
        ...
        *,
        ...
        # ball_vel_dir already computed above; goal_dir already computed above.
        # Positive dot → ball moving toward opponent goal → reward fires.
        alignment = float(np.dot(ball_vel_dir, goal_dir))
        shooting_reward = float(
            np.clip(0.04 * max(0.0, alignment) * ball_speed, 0.0, 0.06)
        )
...
def build_config(args) -> Dict[str, Any]:
    ...
    return {
        ...
        # Fix 2 — separate value head so policy/value gradients don't conflict
        "model": {
            "vf_share_layers": False,       # was True in v1
        ...
        "lambda": 0.95,
        "clip_param": 0.2,
        "entropy_coeff": 0.005,             # raised to 0.015 in stage 3 via callback (Fix 3)
        "vf_loss_coeff": 0.5,              # was 1.0 in v1 (Fix 2)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Three-phase curriculum PPO v2 — separate VF head, higher entropy S3, "
            "shooting reward (stage 3 only, sign-corrected).\n\n"
            "IMPORTANT: vf_share_layers=False makes this INCOMPATIBLE with checkpoints "
            "from v1 (vf_share_layers=True). This is a fresh-run-only script. "
            "Do not pass a v1 checkpoint as a restore path."
        )
    )
    ...
    parser.add_argument("--phase3-timesteps", type=int, default=3_000_000,
                        help="Extended to 3M (was 2.5M) to give more time against baseline.")
    parser.add_argument("--num-workers", type=int, default=2)
    parser.add_argument("--num-envs-per-worker", type=int, default=1)
    parser.add_argument("--experiment-name", type=str, default="ppo_curriculum_v2_fixed")
...
def main():
    ...
    print(
        "\n[v2] Architecture: vf_share_layers=False (separate value head).\n"
        "[v2] This run is NOT compatible with checkpoints from v1 (vf_share_layers=True).\n"
        "[v2] Starting fresh from scratch.\n"
    )
